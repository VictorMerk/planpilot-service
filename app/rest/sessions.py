...
def validate_create_session_payload(payload):
    ...
    if type(configuration.get("stateFacets", False)) is not bool:
        return "configuration.stateFacets must be a boolean."
...
def validate_query_payload(payload):
    ...
    solution_mode = payload.get("solutionMode")
    if solution_mode is not None and solution_mode not in {"single", "prefix"}:
        return "solutionMode must be single or prefix."

    if payload.get("type") != "solution" and solution_mode is not None:
        return "solutionMode is only supported for solution queries."
...
def get_configuration(configuration):
    return SessionConfiguration(
        ...
        state_facets=configuration.get("stateFacets", False),
    )
