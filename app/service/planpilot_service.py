...
class PlanpilotService:
    ...
    def run_planpilot_service(
        ...
        if type(state_facets) is not bool:
            raise ValueError("state_facets must be a boolean.")
        ...
        self.last_state_facets = state_facets
    ...
    def restart_FASB(self, timeout_seconds=None):
        if (
            ...
            or self.last_state_facets is None
    ...
    def _generate_lp_with_plasp(
        ...
        with open(lp_output_path, "w") as lp_file:
            ...
            if state_facets:
                state_facet_file = os.path.join(encoding_dir, "state-facets.lp")
                with open(state_facet_file, "r") as sf:
                    lp_file.write(sf.read())
    ...
    def _lp_file_path(
        ...
        facet_mode = "state" if state_facets else "actions"
        return os.path.join(
            current_directory,
            "temp",
            hash_value,
            f"output-{encoding}-{time_mode}-{facet_mode}.lp",
        )
